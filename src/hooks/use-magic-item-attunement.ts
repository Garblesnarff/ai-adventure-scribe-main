...
export const useMagicItemAttunement = (character: Character, onCharacterUpdate: (updatedCharacter: Character) => void) => {
  const [isAttuning, setIsAttuning] = useState(false);

  /**
   * Attempt to attune to a magic item
   */
  const attuneToItem = useCallback(async (itemId: string): Promise<AttunementResult> => {
    setIsAttuning(true);
   ...
   */
  const removeAttunement = useCallback(async (itemId: string): Promise<AttunementResult> => {
    try {
      // Find the item in character inventory
      const item = character.inventory?.find(invItem => invItem.itemId === itemId);
      if (!item) {
   ...
   */
  const getItemAttunementStatus = useCallback((itemId: string) => {
    const item = character.inventory?.find(invItem => invItem.itemId === itemId);
