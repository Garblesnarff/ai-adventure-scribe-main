...
export const useMulticlassing = (character: Character, onCharacterUpdate: (updatedCharacter: Character) => void) => {
   ...
   */
  const validateNewClass = useCallback(async (newClass: CharacterClass): Promise<MulticlassValidationResult> => {
   ...
   */
  const levelUpSpecificClass = useCallback(async (classId: string): Promise<MulticlassingResult> => {
    setIsProcessing(true);
    ...
    try {
      ...
      const className = updatedCharacter.classLevels?.find(cls => cls.classId === classId)?.className || 'Unknown';
   ...
   */
  const getClassLevel = useCallback((classId: string) => {
