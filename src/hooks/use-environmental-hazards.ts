...
export const useEnvironmentalHazards = (character: Character) => {
   ...
   * Detect a hazard
   */
  const detectHazardById = useCallback(async (hazardId: string): Promise<HazardDetectionResult> => {
    ...
    try {
      ...
      setHazardInteractions(prev => {
        const existing = prev.find(i => i.hazardId === hazardId && i.characterId === character.id);
        if (existing) {
          return prev.map(i => 
   ...
   */
  const triggerHazard = useCallback(async (hazardId: string): Promise<HazardSaveResult> => {
    setIsProcessing(true);
   ...
   * Get hazard by ID
   */
  const getHazardById = useCallback((hazardId: string): EnvironmentalHazard | undefined => {
   ...
   */
  const getHazardInteractionStatus = useCallback((hazardId: string) => {
    const interaction = hazardInteractions.find(i => 
      i.hazardId === hazardId && i.characterId === character.id
    );
  ...
  const getDetectedHazards = useCallback((): EnvironmentalHazard[] => {
    return activeHazards.filter(hazard => {
      const interaction = hazardInteractions.find(i => 
        i.hazardId === hazard.id && i.characterId === character.id
      );
