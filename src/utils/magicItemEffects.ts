 ...
 */
export function getMagicAbilityBonuses(character: Character): Partial<Record<'strength' | 'dexterity' | 'constitution' | 'intelligence' | 'wisdom' | 'charisma', number>> {
  if (!character.inventory) return {};
  
  const bonuses: Partial<Record<'strength' | 'dexterity' | 'constitution' | 'intelligence' | 'wisdom' | 'charisma', number>> = {};
...
export function validateAttunementRequirements(character: Character, item: any): { canAttune: boolean; reason: string } {
  ...
  // Check class requirements
  if (item.attunementRequirements && item.attunementRequirements.includes('class:') && character.class) {
    const requiredClasses = item.attunementRequirements
      .split('class:')[1]
      .split(',')[0]
      .split('|')
  ...
  // Check race requirements
  if (item.attunementRequirements && item.attunementRequirements.includes('race:') && character.race) {
    const requiredRaces = item.attunementRequirements
      .split('race:')[1]
      .split(',')[0]
      .split('|')
  ...
  // Check alignment requirements
  if (item.attunementRequirements && item.attunementRequirements.includes('alignment:') && character.alignment) {
    const requiredAlignments = item.attunementRequirements
      .split('alignment:')[1]
      .split(',')[0]
      .split('|')
