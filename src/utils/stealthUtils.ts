...
export function calculateStealthBonus(participant: CombatParticipant): number {
  ...
  return dexModifier + proficiencyBonus + racialBonus + classBonus + equipmentBonus + conditionModifier;
}
 ...
 */
export function canRemainHidden(
  participant: CombatParticipant,
  actionTaken: string
): boolean {
...
export function canSeeHidden(
  ...
  const hasBlindsight = observer.visionTypes?.some(v => v.type === 'blindsight' && distance <= v.range);
  const hasTruesight = observer.visionTypes?.some(v => v.type === 'truesight' && distance <= v.range);
