...
import { RacialTrait, DamageType } from '@/types/combat';
  ...
  const raceTraits: Record<string, RacialTrait[]> = {
    ...
    'tiefling': [
      ...
      },
      {
        ...
        description: 'You know the thaumaturgy cantrip. You can cast hellish rebuke once per day at 2nd level, and darkness once per day at 3rd level.',
        type: 'active',
        usesPerRest: 'long',
        maxUses: 2,
        currentUses: 2
 ...
 */
export function getBreathWeaponSaveDC(constitutionModifier: number, proficiencyBonus: number): number {
 ...
 */
export function restoreRacialTraits(traits: RacialTrait[], restType: 'short' | 'long'): RacialTrait[] {
