...
import { CombatParticipant, CombatAction, DamageType } from '@/types/combat';
...
export function canUseTwoWeaponFighting(participant: CombatParticipant): boolean {
  ...
  const offHandIsShield = offHandWeapon.category === 'shield' || offHandWeapon.name?.toLowerCase().includes('shield');
...
export function makeMainHandAttack(
  ...
  const diceTotal = damageRolls.reduce((sum, dr) => sum + dr.keptResults.reduce((a, b) => a + b, 0) + dr.modifier, 0);
  const totalDamage = diceTotal + abilityModifier + (mainWeapon.weaponProperties?.magical ? (mainWeapon.magicBonus || 0) : 0);
...
export function makeOffHandAttack(
  ...
  const totalDamage = diceTotal + abilityBonus + (offHandWeapon.weaponProperties?.magical ? (offHandWeapon.magicBonus || 0) : 0);
