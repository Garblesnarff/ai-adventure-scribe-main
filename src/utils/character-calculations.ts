...
export interface CharacterStats {
  ...
  skillModifiers: { [skill: string]: { modifier: number; proficient: boolean; expertise: boolean } };
 ...
 */
export const getSpellcastingAbility = (characterClass: CharacterClass | null): keyof Character['abilityScores'] | null => {
 ...
 */
export const calculateSpellSlots = (character: Character): { [level: number]: number } | undefined => {
  const spellcastingAbility = getSpellcastingAbility(character.class);
  if (!spellcastingAbility) return undefined;
 ...
 */
export const getClassSavingThrowProficiencies = (characterClass: CharacterClass | null): string[] => {
  if (!characterClass) return [];
