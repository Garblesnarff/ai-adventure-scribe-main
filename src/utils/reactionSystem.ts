...
export function checkAbsorbElementsOpportunities(
  ...
  if (canCastAbsorbElements(target, damageType) && !target.reactionTaken && target.currentHitPoints > 0) {
    opportunities.push(createReactionOpportunity(
      target.id,
      'damage_taken',
...
export function checkReactionTriggers(
  ...
  switch (action.actionType) {
    case 'attack':
      ...
      if (action.movement && action.movement.fromPosition && action.movement.toPosition) {
        ...
        if (movingParticipant) {
          opportunities.push(...checkOpportunityAttacks(
            movingParticipant,
            encounter,
      ...
      if (action.hit && action.targetParticipantId) {
        ...
        const allies = encounter.participants.filter(p => 
    ...
    case 'cast_spell':
      ...
      if (caster) {
        opportunities.push(...checkCounterspellOpportunities(
          caster,
          encounter,
          action.spellLevel || 1
        ));
    ...
    case 'damage_dealt':
      ...
      if (attacker && target) {
        ...
        if (action.damageType) {
          const absorbElementsOpportunities = checkAbsorbElementsOpportunities(target, encounter, action.damageType);
        ...
        if (attacker.participantType !== target.participantType) {
          const hellishRebukeOpportunities = checkHellishRebukeOpportunities(target, attacker, encounter);
    ...
    case 'move':
      ...
      if (action.fromPosition && action.toPosition) {
        ...
        if (movingParticipant) {
          ...
          const polearmMasters = encounter.participants.filter(p => 
            p.id !== action.participantId &&
            hasPolearmMaster(p) &&
            !p.reactionTaken &&
            p.currentHitPoints > 0
