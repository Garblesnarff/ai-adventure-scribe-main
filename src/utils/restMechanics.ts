 ...
 */
export function rollHitDice(character: Character, numDice: number): {
  hitPointsRecovered: number;
 ...
 */
export function recoverSpellSlotsShortRest(character: Character, className: string, level: number): Character {
...
export function restoreClassFeaturesOnRest(character: Character, restType: RestType): Character {
  ...
  const currentResources = character.classResources || getCharacterResources(
 ...
 */
export function recoverExhaustion(character: Character, hadFoodAndWater: boolean = true): Character {
...
export function processLongRest(character: Character, hadFoodAndWater: boolean = true): RestResult {
  ...
  if (charWithHitDice.hitDice?.remaining !== updatedCharacter.hitDice?.remaining) {
    hitDiceRecovered = (charWithHitDice.hitDice?.remaining || 0) - (updatedCharacter.hitDice?.remaining || 0);
  }
 ...
 */
export function processShortRestCombat(participant: CombatParticipant, hitDiceToRoll: number = 0): CombatParticipant {
