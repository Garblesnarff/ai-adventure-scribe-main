...
vi.mock('@/services/spellApi', () => {
  return {
    spellApi: {
      calculateMulticlassCasterLevel: vi.fn(async (classLevels: { className: string; level: number }[]) => {
...
describe('spell-validation async/multiclass utilities', () => {
  ...
  beforeEach(() => {
    wizard = {
      id: 'wizard', name: 'Wizard', description: 'Arcane', hitDie: 6,
      primaryAbility: 'intelligence', savingThrowProficiencies: ['intelligence', 'wisdom'], skillChoices: [], numSkillChoices: 2,
      spellcasting: { ability: 'intelligence', cantripsKnown: 3, spellsKnown: 6, ritualCasting: true, spellbook: true },
      classFeatures: [], armorProficiencies: [], weaponProficiencies: []
    };
    fighter = {
      id: 'fighter', name: 'Fighter', description: 'Warrior', hitDie: 10,
      primaryAbility: 'strength', savingThrowProficiencies: ['strength', 'constitution'], skillChoices: [], numSkillChoices: 2,
      classFeatures: [], armorProficiencies: [], weaponProficiencies: []
    } as any;
    warlock = {
      id: 'warlock', name: 'Warlock', description: 'Pact', hitDie: 8,
      primaryAbility: 'charisma', savingThrowProficiencies: ['wisdom', 'charisma'], skillChoices: [], numSkillChoices: 2,
      spellcasting: { ability: 'charisma', cantripsKnown: 2, spellsKnown: 2, ritualCasting: false } as any,
      classFeatures: [], armorProficiencies: [], weaponProficiencies: []
    };
    human = { id: 'human', name: 'Human', description: '', abilityScoreIncrease: {}, speed: 30, traits: [], languages: ['Common'] };
    tiefling = { id: 'tiefling', name: 'Tiefling', description: '', abilityScoreIncrease: { charisma: 2 }, traits: ['Infernal Legacy'], cantrips: ['thaumaturgy'] };
  });
  ...
  it('isSpellValidForClassAsync returns true on ok response', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, json: async () => ({ isValid: true }) })) as any);
  ...
  it('calculateMulticlassCasterLevel aggregates levels', async () => {
    const calc = await calculateMulticlassCasterLevel([{ className: 'Wizard', level: 1 }, { className: 'Warlock', level: 2 }]);
  ...
  it('getEnhancedSpellcastingInfo returns base with multiclass info for multiclass char', async () => {
    const ch: Character = {
      id: 'c', name: 'MC', level: 3, class: wizard, race: human,
      abilityScores: {
        strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
        constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 14, modifier: 2, savingThrow: false },
        wisdom: { score: 12, modifier: 1, savingThrow: false }, charisma: { score: 12, modifier: 1, savingThrow: false }
      },
      classLevels: [
  ...
  it('validateMulticlassSpellSelection returns warnings and valid for multiclass', async () => {
    ...
    const res = await validateMulticlassSpellSelection(ch, ['mage-hand', 'prestidigitation', 'light'], ['magic-missile', 'shield', 'detect-magic', 'burning-hands', 'sleep', 'color-spray']);
  ...
  it('validateMulticlassSpellSelection remains valid but warns when calculation fails', async () => {
    const spy = (await import('@/services/spellApi')).spellApi.calculateMulticlassCasterLevel as any;
    spy.mockRejectedValueOnce(new Error('fail'));
    const ch: Character = {
      id: 'c2', name: 'Bad', level: 3, class: wizard, race: human,
      abilityScores: {
        strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
        constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 14, modifier: 2, savingThrow: false },
        wisdom: { score: 12, modifier: 1, savingThrow: false }, charisma: { score: 12, modifier: 1, savingThrow: false }
  ...
  it('validateMulticlassSpellSelection falls back for single-class', async () => {
    const ch: Character = {
      id: 'c3', name: 'Single', level: 1, class: wizard, race: human,
      abilityScores: {
        strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
        constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 14, modifier: 2, savingThrow: false },
        wisdom: { score: 12, modifier: 1, savingThrow: false }, charisma: { score: 12, modifier: 1, savingThrow: false }
  ...
  describe('validateSpellSelectionAsync', () => {
    it('validates non-spellcaster with racial cantrip', async () => {
      const ch: Character = {
        id: 'f1', name: 'Tiefling Fighter', level: 1, class: fighter, race: human, subrace: tiefling,
        abilityScores: {
          strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
          constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 10, modifier: 0, savingThrow: false },
          wisdom: { score: 10, modifier: 0, savingThrow: false }, charisma: { score: 10, modifier: 0, savingThrow: false }
        }
      } as any;
    ...
    it('rejects racial cantrip count mismatch for non-spellcaster', async () => {
      const ch: Character = {
        id: 'f2', name: 'Tiefling Fighter', level: 1, class: fighter, race: human, subrace: tiefling,
        abilityScores: {
          strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
          constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 10, modifier: 0, savingThrow: false },
          wisdom: { score: 10, modifier: 0, savingThrow: false }, charisma: { score: 10, modifier: 0, savingThrow: false }
    ...
    it('flags invalid cantrip with availableCantripIds', async () => {
      const ch: Character = {
        id: 'w1', name: 'Wizard', level: 1, class: wizard, race: human,
        abilityScores: {
          strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
          constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 14, modifier: 2, savingThrow: false },
          wisdom: { score: 12, modifier: 1, savingThrow: false }, charisma: { score: 10, modifier: 0, savingThrow: false }
        }
      };
      const res = await validateSpellSelectionAsync(ch, ['mage-hand', 'prestidigitation', 'guidance'], [], ['mage-hand', 'prestidigitation'], []);
      expect(res.valid).toBe(false);
      expect(res.errors.some(e => e.type === 'INVALID_SPELL' && e.spellId === 'guidance')).toBe(true);
    });

    it('enforces spells known count and invalid spell with availableSpellIds', async () => {
      const ch: Character = {
        id: 'w2', name: 'Wizard', level: 1, class: wizard, race: human,
        abilityScores: {
          strength: { score: 10, modifier: 0, savingThrow: false }, dexterity: { score: 10, modifier: 0, savingThrow: false },
          constitution: { score: 10, modifier: 0, savingThrow: false }, intelligence: { score: 14, modifier: 2, savingThrow: false },
          wisdom: { score: 12, modifier: 1, savingThrow: false }, charisma: { score: 10, modifier: 0, savingThrow: false }
      ...
      const res = await validateSpellSelectionAsync(ch, ['mage-hand', 'prestidigitation', 'light'], spells, ['mage-hand', 'prestidigitation', 'light'], ['magic-missile', 'shield']);
      expect(res.valid).toBe(false);
      expect(res.errors.some(e => e.type === 'COUNT_MISMATCH')).toBe(true);
