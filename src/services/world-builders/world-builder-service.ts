import { LocationGenerator, LocationRequest, GeneratedLocation } from './location-generator';
...
export class WorldBuilderService {
  ...
  static async analyzeBuildingNeeds(context: WorldBuildingContext): Promise<WorldBuildingTrigger> {
    ...
    const locationTriggers = [
      'go to', 'enter', 'travel to', 'visit', 'explore', 'find', 'search for'
    ];
    ...
    const npcTriggers = [
      'talk to', 'speak with', 'meet', 'find someone', 'ask', 'hire', 'buy from'
    ];
    ...
    const questTriggers = [
      'help', 'quest', 'mission', 'task', 'job', 'problem', 'trouble'
    ];
    ...
    const memoryBasedOpportunities = recentMemories.filter(memory => 
      memory.type === 'quest' || 
      memory.type === 'npc' || 
  ...
  static async expandWorld(context: WorldBuildingContext): Promise<WorldExpansionResult> {
    ...
    try {
      ...
      if (buildingNeeds.suggestions.quests || result.npcs.length > 0 || result.locations.length > 0) {
        try {
  ...
  private static inferQuestTypeFromAction(action: string): QuestRequest['type'] {
    ...
    if (actionLower.includes('talk') || actionLower.includes('negotiate') || actionLower.includes('convince')) {
      return 'social';
    }
    if (actionLower.includes('find') || actionLower.includes('get') || actionLower.includes('bring')) {
      return 'fetch';
    }
    if (actionLower.includes('kill') || actionLower.includes('defeat') || actionLower.includes('fight')) {
      return 'kill';
    }
    if (actionLower.includes('escort') || actionLower.includes('protect') || actionLower.includes('guard')) {
      return 'escort';
    }
    if (actionLower.includes('explore') || actionLower.includes('discover') || actionLower.includes('map')) {
      return 'exploration';
    }
  ...
  static async getWorldStats(campaignId: string) {
    try {
      ...
      return {
        ...
        totalElements: (locations.data?.length || 0) + (npcs.data?.length || 0) + (quests.data?.length || 0),
      };
