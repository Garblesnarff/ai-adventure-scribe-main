...
describe('RollManager (flag‑gated)', () => {
  ...
  it('does nothing when VITE_ENABLE_ROLL_HISTORY=false', async () => {
    ...
    await RollManager.recordRollRequest({ sessionId: 's1', kind: 'check', purpose: 'Stealth check' });
  ...
  it.skip('writes request when flag enabled', async () => {
    ...
    const { RollManager } = await import('@/services/roll-manager');
    await RollManager.recordRollRequest({ sessionId: 's1', kind: 'attack', purpose: 'Attack roll', formula: '1d20+5', ac: 13 });
    expect(supabaseMock.inserts.length).toBe(1);
    expect(supabaseMock.inserts[0]).toMatchObject({ session_id: 's1', kind: 'attack', formula: '1d20+5', ac: 13 });
  });
