 ...
 */
function generateRecommendations(violations: RuleViolation[], summary: AuditReport['summary']): string[] {
  ...
  if (!summary.formulaCompliance) {
    recommendations.push('🎯 Include modifiers in dice formulas: "1d20+5" not "1d20", "1d8+3" not "1d8".');
  }
  ...
  if (violations.some(v => v.severity === 'critical')) {
    recommendations.push('🚨 Address critical rule violations first, as they break core game mechanics.');
  }
...
export function generateAuditReport(
  ...
  const endTime = isCombatActive ? undefined : (actions.length > 0 ? Math.max(...actions.map(a => a.timestamp)) : Date.now());
