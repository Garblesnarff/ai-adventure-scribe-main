...
const APPEARANCE_BY_RACE: Partial<Record<RaceKey, AppearanceOptions>> = {
  ...
  dragonborn: {
    ...
    skinColors: ['black', 'blue', 'brass', 'bronze', 'copper', 'gold', 'green', 'red', 'silver', 'white'],
    hairColors: ['crest - black', 'crest - blue', 'crest - bronze', 'crest - copper', 'crest - gold', 'crest - green', 'crest - red', 'crest - silver', 'crest - white']
  }
};
...
function mergeOptions(base: AppearanceOptions, override?: Partial<AppearanceOptions>): AppearanceOptions {
...
export function getAppearanceOptions(raceName?: string, subraceId?: string): AppearanceOptions {
  ...
  const base = raceKey && APPEARANCE_BY_RACE[raceKey] ? APPEARANCE_BY_RACE[raceKey]! : defaultOptions;
  const subraceKey = normalizeKey(subraceId);
  const subraceOptions = subraceKey && APPEARANCE_BY_RACE[subraceKey] ? APPEARANCE_BY_RACE[subraceKey] : undefined;
