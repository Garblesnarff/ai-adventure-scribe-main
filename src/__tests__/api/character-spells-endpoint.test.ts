...
describe('Character Spells API Endpoint', () => {
  ...
  describe('POST /characters/:id/spells - Spell Validation', () => {
    it('should reject wizard selecting cleric spells', async () => {
      ...
      mockSupabaseService.from().select().eq().eq().single.mockResolvedValueOnce({
        data: { id: 'char-123', class: 'Wizard' },
        error: null
      });

      // Mock class lookup
      mockSupabaseService.from().select().eq().single.mockResolvedValueOnce({
      ...
      mockSupabaseService.from().select().eq().eq().single
        .mockResolvedValueOnce({ data: null, error: { code: 'PGRST116' } }) // cure-wounds not in wizard list
    ...
    it('should reject cleric selecting wizard spells', async () => {
      ...
      mockSupabaseService.from().select().eq().eq().single
        .mockResolvedValueOnce({ data: null, error: { code: 'PGRST116' } }) // magic-missile not in cleric list
    ...
    it('should accept valid wizard spell selection', async () => {
      ...
      mockSupabaseService.from().select().eq().eq().single
        .mockResolvedValueOnce({ data: { id: 'spell-relation-1' }, error: null }) // magic-missile valid
    ...
    it('should accept valid cleric spell selection', async () => {
      ...
      mockSupabaseService.from().select().eq().eq().single
        .mockResolvedValueOnce({ data: { id: 'spell-relation-3' }, error: null }) // cure-wounds valid
    ...
    it('should reject requests with missing required fields', async () => {
      const testCases = [
        ...
        { body: { spells: ['magic-missile'] }, expectedError: 'Missing required fields: spells and className' },
        { body: { className: 'Wizard' }, expectedError: 'Missing required fields: spells and className' },
        { body: { spells: null, className: 'Wizard' }, expectedError: 'Missing required fields: spells and className' },
        { body: { spells: ['magic-missile'], className: null }, expectedError: 'Missing required fields: spells and className' }
      ];
    ...
    it('should properly clear and insert spells', async () => {
      ...
      mockSupabaseService.from().select().eq().eq().single
        .mockResolvedValueOnce({ data: { id: 'spell-relation-1' }, error: null })
