...
describe('PlayerIntentDetector', () => {
  ...
  it('should detect dialogue intent for messages containing dialogue keywords', () => {
    expect(detector.detectIntent("I want to talk to the bartender.")).toBe<PlayerIntent>('dialogue');
  ...
  it('should detect exploration intent for messages containing exploration keywords', () => {
    ...
    expect(detector.detectIntent("Search for clues near the desk.")).toBe<PlayerIntent>('exploration');
    expect(detector.detectIntent("Can I investigate the strange markings?")).toBe<PlayerIntent>('exploration');
  });
  ...
  it('should handle messages with mixed keywords (first match wins based on current implementation)', () => {
    ...
    expect(detector.detectIntent("I want to talk about exploring the map.")).toBe<PlayerIntent>('dialogue');
    expect(detector.detectIntent("Let's explore and then ask questions.")).toBe<PlayerIntent>('exploration'); // Dialogue keyword "ask" appears after "explore"
  });
