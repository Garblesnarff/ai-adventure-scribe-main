...
async function run() {
  ...
  try {
    ...
    console.log('Starting Bard spell migration...');

    // 1. Insert Bard class
    console.log('Inserting Bard class...');
    const classResult = await client.query(
      `INSERT INTO classes (name, hit_die, spellcasting_ability, caster_type, spell_slots_start_level, ritual_casting, spellcasting_focus_type)
       ...
       RETURNING id`,
      [bardClass.name, bardClass.hit_die, bardClass.spellcasting_ability, bardClass.caster_type,
       bardClass.spell_slots_start_level, bardClass.ritual_casting, bardClass.spellcasting_focus_type]
    );

    const bardClassId = classResult.rows[0].id;
    ...
    for (const spell of allBardSpells) {
      const spellResult = await client.query(
        `INSERT INTO spells (
          ...
          damage_at_slot_level, heal_at_slot_level, damage_type, area_of_effect
      ...
      spellIds[spell.name] = spellResult.rows[0].id;
    }
    ...
    for (const spell of allBardSpells) {
      await client.query(
        ...
        [bardClassId, spellIds[spell.name], spell.level, 'base']
      );
    }

    // 4. Insert spell progression data
    console.log('Inserting spell progression data...');
    for (const progression of bardSpellProgression) {
      await client.query(
        ...
        [
          bardClassId, progression.character_level, progression.cantrips_known, progression.spells_known,
          progression.spell_slots_1 || 0, progression.spell_slots_2 || 0, progression.spell_slots_3 || 0,
          progression.spell_slots_4 || 0, progression.spell_slots_5 || 0, progression.spell_slots_6 || 0,
          progression.spell_slots_7 || 0, progression.spell_slots_8 || 0, progression.spell_slots_9 || 0
        ]
      );
    }
