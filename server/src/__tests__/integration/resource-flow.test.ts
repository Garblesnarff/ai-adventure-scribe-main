...
describe('Spell Slot and Rest Flow', () => {
  ...
  test('cast spell → use slot → short rest → slot remains used → long rest → slot restored', async () => {
    ...
    const longRestResult = await RestService.takeLongRest({
      characterId: wizard.id,
    });
  ...
  test('hit dice: spend during short rest → recover on long rest', async () => {
    ...
    const shortRest = await RestService.takeShortRest({
      ...
      hitDiceToSpend: [
        { className: 'Fighter', count: 2 }
      ],
    });

    expect(shortRest.hitDiceSpent.length).toBeGreaterThan(0);
    expect(shortRest.hitDiceSpent[0].diceSpent).toBe(2);
    ...
    expect(diceAfterSecond?.remainingDice).toBe(2);

    // Take long rest (should recover half of total hit dice, rounded down)
    const longRest = await RestService.takeLongRest({
      characterId: fighter.id,
    });

    expect(longRest.hitDiceRestored.length).toBeGreaterThan(0);
    ...
    expect(diceAfterLong?.remainingDice).toBe(4); // 5 - 1 = 4
  }, 30000);
  ...
  test('multiple long rests restore all resources', async () => {
    ...
    await RestService.takeShortRest({
      characterId: wizard.id,
      hitDiceToSpend: [{ className: 'Wizard', count: 3 }],
    });
    ...
    // First long rest
    await RestService.takeLongRest({ characterId: wizard.id });
  ...
  test('spell slot usage is tracked with history', async () => {
    ...
    // Get usage history
    const history = await SpellSlotsService.getUsageHistory({
      characterId: wizard.id,
      limit: 10,
    });

    expect(history.length).toBe(3);
    expect(history[0].spellName).toBe('Counterspell'); // Most recent first
    expect(history[1].spellName).toBe('Fireball');
    expect(history[2].spellName).toBe('Magic Missile');

    // Verify slot levels
    expect(history.filter(h => h.slotLevel === 1).length).toBe(1);
    expect(history.filter(h => h.slotLevel === 3).length).toBe(2);
  }, 30000);
  ...
  test('short rest duration is tracked', async () => {
    ...
    const restResult = await RestService.takeShortRest({
      ...
      hitDiceToSpend: [],
      durationMinutes: 60, // Standard 1-hour short rest
    });

    expect(restResult.restType).toBe('short');
    expect(restResult.durationMinutes).toBe(60);
